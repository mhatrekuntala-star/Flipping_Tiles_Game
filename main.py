...
def displayBoard():
    ...
    for i in range(4):
        for j in range(4):
            rect=base.create_rectangle(100*i,j*100,100*i+100,100*j+100,fill="sky blue")
            if(board[i][j]!='.'):
                draw(board[i][j],i,j)
                cnt+=1
    if cnt==16:
        base.create_text(200,450,text="GameOver \n Moves: "+str(moves),font=('arial',25))
...
def displayboard2():
    ...
    for i in range(6):
        for j in range(6):
            rect=base2.create_rectangle(100*i,j*100,100*i+100,100*j+100,fill="pink")
            if(board2[i][j]!='.'):
                draw2(board2[i][j],i,j)
                cnt+=1
    if cnt>=36:
        #print("cnt------------------------",cnt)
        base2.create_text(250,550,text=" Game Over /n Moves: "+str(moves2),font=('arial',20))
